...
# Database
DATABASE_URL = "postgresql+psycopg2://neutrino:123@localhost:5432/yourdatabase"
...
def save_analysis_to_db(analysis_data: AnalysisData):
    db = SessionLocal()
    db_analysis = AgentResponse(
        ...
        business_strategy=analysis_data.business_strategy
    )
    db.add(db_analysis)
    db.commit()
...
if st.button("Submit"):
    if user_input_company_name:
        ...
        save_analysis_to_db(analysis_data)
